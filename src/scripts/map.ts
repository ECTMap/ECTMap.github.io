...
export function routePopupHTML(p) {
  const length = p.length_km_calc ?? p.length_km;
  return `
    <div class="popup-header">
      <span class="popup-header-title">${p.title ?? p.category}</span>
    </div>
    <div class="popup-body">
      ${p.type       ? `<p class="popup-row">${p.type}</p>` : ''}
      ${p.difficulty ? `<p class="popup-row">${p.difficulty}</p>` : ''}
      ${length != null ? `<p class="popup-row">${length} km</p>` : ''}
    </div>
  `.trim();
}
