...
function _initResize() {
    ...
    const handle = document.getElementById('route-panel-resize-handle') as HTMLElement;
    ...
    handle.addEventListener('mousedown', (e) => {
        ...
        startH = panel.getBoundingClientRect().height / scale;
        document.body.style.userSelect = 'none';

        function onMove(ev: MouseEvent) {
            const delta = startY - ev.clientY;
            const newH  = Math.min(Math.max(startH + delta, 325), window.innerHeight * 0.80);
    ...
    const leftHandle = document.getElementById('route-panel-resize-handle-left') as HTMLElement;
    if (leftHandle) {
        let startX = 0;
        let startW = 0;
        ...
        leftHandle.addEventListener('mousedown', (e) => {
            ...
            function onMove(ev: MouseEvent) {
                ...
                const newW  = Math.min(Math.max(startW + delta, 375), window.innerWidth * 0.80);
...
export function init(onClose?: () => void) {
    ...
    if (window.matchMedia('(pointer: fine)').matches) _initResize();
}
